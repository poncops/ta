...
class OrderController extends Controller
{
	...
	public function setShipping(Request $request)
	{
		...
		if ($selectedShipping) {
			...
			$data['cost'] = $selectedShipping['cost'];
			$data['courier'] = $selectedShipping['courier'];
			$data['etd'] = $selectedShipping['etd'];
			$data['service'] = $selectedShipping['service'];
		} else {
			$status = 400;
	...
	public function checkout(Request $request){
		...
		$order = \DB::transaction(function() use ($params) {
			...
			$orderParams = [
				...
				'shipping_service_name' => $params['shipping_service'],
			];
